...
real SampleShadow_PCSS(ShadowContext shadowContext, inout uint payloadOffset, real3 coord, real3 positionWS, real4 scaleOffset, float slice, Texture2DArray tex, SamplerComparisonState compSamp, SamplerState samp)
{
	...
	real2 Noise = SampleNoise(positionWS);

	//1) Blocker Search
	real AverageBlockerDepth = 0.0;
	real NumBlockers 		 = 0.0;
	if(!BlockerSearch(AverageBlockerDepth, NumBlockers, LightArea, Noise, coord, slice, tex, samp)) 
		return 1.0;
	...
	//3) Filter
	return PCSS(coord, FilterSize, Noise, scaleOffset, slice, tex, compSamp); 
}
